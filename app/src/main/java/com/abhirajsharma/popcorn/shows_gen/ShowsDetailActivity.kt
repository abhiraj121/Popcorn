...
class ShowsDetailActivity: AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        show = intent.getParcelableExtra("shows") as TvShows?
        showName = show?.originalName
        Log.d("showname","$showName is fetched")
        plotsynopsis.text = show?.overview.toString()
        userrating.text = show?.voteAverage.toString()
        releasedate.text = show?.firstAirDate.toString()
        showId = show?.id
        val poster = "https://image.tmdb.org/t/p/w500${show?.posterPath}"
...
//                } else {
//                    val favoriteDbHelper = FavoriteDbHelper(this@ShowsDetailActivity)
//                    favoriteDbHelper.deleteFavorite(showId!!)
...
//                    Snackbar.make(buttonView, "Added to Favorite", Snackbar.LENGTH_SHORT).show()
//                } else {
//                    showId = intent.extras!!.getInt("id")
//                    val favoriteDbHelper = FavoriteDbHelper(this@ShowsDetailActivity)
//                    favoriteDbHelper.deleteFavorite(showId!!)
//                    Snackbar.make(buttonView, "Removed from Favorite", Snackbar.LENGTH_SHORT).show()
//                }
...
//        }
    ...
    private fun loadJSON() {
        try{
            ...
            val call: Call<TrailerResponse> = apiService.getShowsTrailer(showId!!, THE_MOVIE_DB_API_TOKEN)
            ...
            call.enqueue(object : Callback<TrailerResponse?> {
                ...
                override fun onFailure(call: Call<TrailerResponse?>, t: Throwable) {
                    ...
                    Toast.makeText(this@ShowsDetailActivity, "Error fetching trailer", Toast.LENGTH_SHORT).show()
                }
            })
